...
public:
	...
	virtual HRESULT updateBoundsWithDpr(RECT& bounds, double dpr) = 0;
	...
	// Read all WebView2 cookies (including httpOnly) for the given uri
	// (empty = current page), concatenated as "name=value; ", returned
	// asynchronously as a UTF-8 string. Used for cookie-session sites
	// (e.g. MiniMax).
	virtual HRESULT getCookies(std::wstring uri, std::function<void(std::string)> callback) = 0;
