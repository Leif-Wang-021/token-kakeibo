...
public:
    ...
    HRESULT updateBoundsWithDpr(RECT& bounds, double dpr);
    ...
    HRESULT getCookies(std::wstring uri, std::function<void(std::string)> callback);
...
// bounds is already in physical pixels (converted by Dart using DPR).
// Sync RasterizationScale so the WebView2 CSS viewport equals logical size.
// Without this, on 125%/150% system scaling the page renders at physical
// pixel size, content is magnified and buttons fall outside the window.
HRESULT MyWebViewImpl::updateBoundsWithDpr(RECT& bounds, double dpr)
{
    m_bounds = bounds;
    HRESULT hr = m_pController->put_Bounds(bounds);
    if (SUCCEEDED(hr) && dpr > 0) {
        wil::com_ptr<ICoreWebView2Controller3> controller3 =
            m_pController.query<ICoreWebView2Controller3>();
        if (controller3 != NULL) {
            controller3->put_RasterizationScale(dpr);
            // Let WebView2 keep following the monitor DPI afterwards
...
HRESULT MyWebViewImpl::getCookies(std::wstring uri, std::function<void(std::string)> callback)
{
    wil::com_ptr<ICoreWebView2CookieManager> cookieManager;
    auto webview2_2 = m_pWebview.try_query<ICoreWebView2_2>();
    if (webview2_2 == NULL) return E_FAIL;
    ...
    // GetCookies(uri, handler): uri is required; use the provided uri,
    // or the current page URL when empty.
    wil::unique_cotaskmem_string source;
    if (uri.empty()) {
        if (FAILED(m_pWebview->get_Source(&source))) return E_FAIL;
    } else {
        source.reset(static_cast<wchar_t*>(
            CoTaskMemAlloc((uri.size() + 1) * sizeof(wchar_t))));
        if (source == NULL) return E_OUTOFMEMORY;
        wcscpy_s(source.get(), uri.size() + 1, uri.c_str());
    }

    return cookieManager->GetCookies(
        source.get(),
        Callback<ICoreWebView2GetCookiesCompletedHandler>(
            [callback](HRESULT result, ICoreWebView2CookieList* cookieList) -> HRESULT {
                if (callback == nullptr) return S_OK;
                std::wstring all;
                if (SUCCEEDED(result) && cookieList != NULL) {
                    unsigned int count = 0;
                    cookieList->get_Count(&count);
                    for (unsigned int i = 0; i < count; i++) {
                        wil::com_ptr<ICoreWebView2Cookie> cookie;
                        if (FAILED(cookieList->GetValueAtIndex(i, &cookie))) continue;
                        ...
                        if (name != NULL) {
                            all += name.get();
                            all += L"=";
                            if (value != NULL) all += value.get();
                            all += L"; ";
                        }
                    }
                }
                callback(utf8_encode(all));
                return S_OK;
            }).Get());
}
