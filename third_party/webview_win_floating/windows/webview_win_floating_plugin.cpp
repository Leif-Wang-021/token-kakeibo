...
void WebviewWinFloatingPlugin::HandleMethodCall(
  ...
  if (isCreateCall) {
    ...
    double dpr = 1.0;
    if (arguments[flutter::EncodableValue("devicePixelRatio")].IsNull() == false) {
      dpr = std::get<double>(arguments[flutter::EncodableValue("devicePixelRatio")]);
    }
    webview->updateBoundsWithDpr(bounds, dpr);
    ...
    // Read all WebView2 cookies (incl. httpOnly) asynchronously, then reply.
    std::wstring uri = L"";
    if (arguments[flutter::EncodableValue("uri")].IsNull() == false) {
      auto s = std::get<std::string>(arguments[flutter::EncodableValue("uri")]);
      if (!s.empty()) uri = utf8ToUtf16(s);
    }
    auto pending = std::make_shared<
        std::unique_ptr<flutter::MethodResult<flutter::EncodableValue>>>(
        std::move(result));
    HRESULT hr = webview->getCookies(
        uri,
        [pending](std::string cookies) {
          if (*pending != nullptr) {
            (*pending)->Success(flutter::EncodableValue(cookies));
            pending->reset(); // release result, prevent double reply
          }
        });
    if (FAILED(hr) && *pending != nullptr) {
      (*pending)->Error("getCookies_failed", "GetCookies failed");
      pending->reset();
    }
