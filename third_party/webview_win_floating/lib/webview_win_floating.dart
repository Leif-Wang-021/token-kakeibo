...
class WinWebViewController {
  ...
  /// 读取 WebView2 全部 cookie（含 httpOnly），返回 "name=value; " 拼接串。
  /// [uri] 为空时用当前页面 URL 作为匹配域。
  Future<String> getCookies({String uri = ''}) async {
    await _initFuture;
    return WebviewWinFloatingPlatform.instance.getCookies(_webviewId, uri: uri);
  }
